...
class StrategicAI:
    ...
    def decide_actions(self):
        ...
        if advantage >= self.ADVANTAGE_THRESHOLD and self.turn_count > 15:
    ...
    def _clear_warehouse_area(self):
        ...
        for wh in whs:
            blocked = [u for u in self.game.all_units
                       ...
                       and abs(u.x - wh.x) + abs(u.y - wh.y) <= 2]
            for unit in blocked:
                if unit.moved:
                    continue
                for _ in range(20):
    ...
    def _do_form_group(self, action):
        ...
        for wi, cl in clusters.items():
            while len(cl['infantry']) >= 2:
                group = BattleGroup(len(self.groups))
                for _ in range(min(3, len(cl['infantry']))):
    ...
    def _do_fortify_group(self, action):
        ...
        if not group.cache:
            ...
            if builder:
                ...
                if wh and abs(builder.x - wh.x) + abs(builder.y - wh.y) < 8:
                    if group.target_pos:
        ...
        if group.cache and group.cache.is_alive:
            if group.cache.build_turns >= group.cache.build_required:
                ...
                fortified = sum(
                    ...
                    self.game.map.get_cell(inf.x, inf.y).entrenchment >= inf.max_entrenchment * 0.5
                )
                if fortified >= max(1, len(group.infantry) // 2):
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if cell and cell.is_walkable and not self._is_occupied(nx, ny):
                ...
                if player_base and self.turn_count >= 10:
                    px = player_base.x + random.randint(-5, 5)
                    py = player_base.y + random.randint(-5, 5)
    ...
    def _manage_drones(self):
        ...
        for drone in drones:
            ...
            if dist_to_target <= 1:
                if player_base:
                    px = player_base.x + random.randint(-8, 8)
                    py = player_base.y + random.randint(-8, 8)
                else:
                    px = random.randint(0, self.game.map.width - 1)
                    py = random.randint(0, self.game.map.height - 1)
                px = max(0, min(self.game.map.width - 1, px))
